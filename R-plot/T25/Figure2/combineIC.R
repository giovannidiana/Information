...
sc_mean <- apply(tabSC,2,function(x) mean(x[!is.infinite(x)]));
sc_var <- apply(tabSC,2,function(x) sd(x[!is.infinite(x)]));
nc_mean <- apply(tabNC,2,function(x) mean(x[!is.infinite(x)]));
nc_var <- apply(tabNC,2,function(x) sd(x[!is.infinite(x)]));

pdf("Fig2.pdf",height=7,width=4)

layout(matrix(c(1,2,3),ncol=1));
par(mar=c(1.5,8,1.5,2))
pos_sc<-barplot(sc_mean,beside=T,cex.axis=2,ylim=c(0,2),
...
par(mar=c(1.5,8,1.5,2))
pos_nc<-barplot(-nc_mean,beside=T,cex.axis=2,ylim=c(-2,0),
