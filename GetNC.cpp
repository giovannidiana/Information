...
// NORMALIZATION and Initialization
	...
	for(l=0;l<gridsize;l++){
		for(k=0;k<6;k++){
			if(joint[k][i][j][l]>0) {
				Info+=joint[k][i][j][l]*input_pdf[k]*log(joint[k][i][j][l]/PmargX[k][i]/PmargY[k][j]/PmargZ[k][l])/log(2.);
				Info_vec[k]+=joint[k][i][j][l]*log(joint[k][i][j][l]/PmargX[k][i]/PmargY[k][j]/PmargZ[k][l])/log(2.);
			}
		}
	}
	}
