...
int main(int argc,char** argv){
	...
	double PmargX[gridsize];
	double PmargY[gridsize];
	double PmargZ[gridsize];
...
// NORMALIZATION and Initialization
	...
	for(j=0;j<gridsize;j++){
	for(l=0;l<gridsize;l++){
		PmargX[i]+=input_pdf[k]*joint[k][i][j][l];
		PmargY[i]+=input_pdf[k]*joint[k][j][i][l];
		PmargZ[i]+=input_pdf[k]*joint[k][j][l][i];
	}}}}
	...
	for(l=0;l<gridsize;l++){
		...
		for(k=0;k<6;k++){
			if(joint[k][i][j][l]>0) {
				Info+=joint[k][i][j][l]*input_pdf[k]*log(PmargOverFood/PmargX[i]/PmargY[j]/PmargZ[l])/log(2.);
				Info_vec[k]+=joint[k][i][j][l]*log(PmargOverFood/PmargX[i]/PmargY[j]/PmargZ[l])/log(2.);
			}
		}
	}
	}
