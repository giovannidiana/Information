...
int main(int argc,char** argv){
	...
	for(i=0;i<6;i++){
		...
		//filename<<argv[2]<<"/pdf3D"<<'_'<<argv[1]<<"_"<<foodlist[i]<<"_GS"<<gridsize<<"_group"<<group<<".dat";
...
// NORMALIZATION
	...
	while(counter<250){
		...
		if(counter==0) {
			for(i=0;i<6;i++) {
				input_pdf[i]=1/6.;
				if(strcmp(argv[1],"QL402")==0){
					input_pdf[i]=input_pdf_402help[i];
				}
		...
		//for(i=0;i<6;i++) input_pdf[i]=max(1e-300,input_pdf[i]/Z);
